...
int t[9][101]; // two variable inputs will be the rows and coloumn of this dp matrix as t[n+1][capacity+1]

// memset sets the value of the 2-d array in one shot
int knapsackMemoization(int wt[], int val[], int size, int capacity)
{
    ...
    if (wt[size - 1] <= capacity)
        return t[size][capacity] = max(val[size - 1] + knapsackMemoization(wt, val, size - 1, capacity - wt[size - 1]), 
                                        knapsackMemoization(wt, val, size - 1, capacity));
...
int main()
{
    ...
    memset(t, -1, sizeof(t)); // this can initialize matrix with 0 or -1 only
